...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/30 00:11:41 by mnosko            #+#    #+#             */
/*   Updated: 2019/10/30 00:11:43 by mnosko           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int								new_window(t_libui *libui, t_vec2 size,
		const char *title);
void							set_window_resizable(t_libui *libui,
		const char *title, int resizable);
void							change_window_surface(t_libui *libui,
		const char *title);
SDL_Window						*find_window(t_libui *libui,
		const char *title);
t_window						*find_t_window(t_libui *libui,
		const char *title);
t_window						*find_window_by_id(t_libui *unicorn,
		Uint32 id);
void							add_button_to_list(t_buttons_list **buttons,
	t_button button,
	void (*click)(void *));
void							add_label_to_list(t_label_list **list,
		t_label label);
...
t_textfield						create_textfield(SDL_Surface *text_surface,
									SDL_Rect rect);
t_label							create_label(char *text,
		t_vec2 pos, t_font *font);
t_button						create_button(SDL_Surface *text_surface,
								SDL_Rect rect, int type, char *name);
void							libui_loop(t_libui *unicorn);
void							eventloop_keydown(t_libui *data,
		int *quit);
void							eventloop_mousebuttondown(t_libui *data);
void							eventloop_window_events(t_libui *data,
		int *quit);
void							libui_hook_m1_down(t_libui *libui,
		void (*func)(t_libui *));
void							libui_hook_m2_down(t_libui *libui,
		void (*func)(t_libui *));
void							libui_hook_m3_down(t_libui *libui,
		void (*func)(t_libui *));
void							libui_hook_mwheel_down(t_libui *libui,
		void (*func)(t_libui *));
void							libui_hook_mwheel_up(t_libui *libui,
		void (*func)(t_libui *));
int								init_libui(t_libui **data);
void							init_widgets(t_widget **widgets);
void							init_keyhooks(t_libui *libui);
t_surface						*create_scaled_area(t_surface *surface,
		double scale,
		t_vec2 topleft, t_vec2 bottomright);
t_vec2							find_scaled_surface_size(t_surface *surface,
		double scale);
...
void							put_pixel(SDL_Surface *img, int x,
		int y, t_color color);
void							fill_surface(SDL_Surface *surface,
		t_color color);
void							flood_fill(t_surface *surface,
		t_vec2 pos,
			t_color affected_color,
			t_color target_color);
...
void							draw_thick_line(
								...
								t_color color,
								int thickness);
void							draw_circle(t_surface *surface,
		t_vec2 center,
		int radius, t_color color);
void							draw_filled_circle(
		t_surface *surface, t_vec2 center,
		int radius, t_color color);
void							draw_rect(t_surface *surface,
		t_vec2 topleft,
								t_vec2 bottomright,
								t_color color);
void							draw_filled_rect(
		t_surface *surface, t_vec2 topleft,
								t_vec2 bottomright, t_color color);
void							put_sticker(t_surface *img,
		t_surface *canvas, t_vec2 pos);
void							draw_line_of_stickers(
		SDL_Surface *surface, t_vec2f start,
			t_vec2f end,
			t_surface *sticker);
void							init_text(t_libui *libui,
		t_text *text);
void							draw_text(t_text *text);
void							add_menu_to_list(t_menu_list **begin,
		t_menu *menu);
...
void							calculate_context_fields_position(
		t_menu_field *field,
		TTF_Font *font, SDL_Rect menu_rect);
void							calculate_bar_fields_position(
		t_menu_field *field,
		TTF_Font *font);
void							menu_events(t_libui *libui,
		t_menu_list *list);
void							draw_menu(SDL_Surface *surface,
		t_menu *menu, TTF_Font *font);
void							draw_all_menus(t_menu_list *list,
		TTF_Font *font);
SDL_Surface						*create_text_surface(char *text,
		TTF_Font *font);
void							draw_rect(t_surface *surface,
		t_vec2 topleft,
								t_vec2 bottomright, t_color color);
void							draw_filled_rect(t_surface *surface,
		t_vec2 topleft,
		t_vec2 bottomright, t_color color);
...
void							draw_sample_box(SDL_Surface *surface,
		SDL_Color color);
void							draw_position_indicator(SDL_Surface *surface,
		t_hsv_color color);
void							draw_hue_slider(SDL_Surface *surface,
		double hue);
void							draw_hue_gradient(SDL_Surface *surface);
void							draw_main_gradient(SDL_Surface *surface,
		double hue);
...
int								point_in_textfield(SDL_Point point,
		t_widget *widgets);
...
void							render_text(t_libui *libui,
		SDL_Event e, t_widget *widgets);

void							blit_with_offset(SDL_Surface *src,
		SDL_Surface *dst, t_vec2 offset);

void							blit_surface(t_surface *src,
		t_int_rect src_rect,
...
void							save_jpg(t_surface *surface, const char *file,
		int quality);
...
void							load_dropped_image(t_libui *unicorn,
		t_surface **target);

void							update_window_surface(t_window_list *list);

void							add_return_data(t_return_data **begin,
		void *data);
...
void							draw_field(SDL_Surface *surface,
		t_menu_field *field,
				TTF_Font *font);
int								pixel_inside_surface(t_surface *img,
		int x, int y);

void							recalculate_menu(t_menu_list *menu,
		TTF_Font *font, int w);
...
void							set_system_cursor(t_cursor *cursor,
		t_libui *libui, int id);
t_surface						*load_image_as_surface(const char *file);
t_surface						*create_text_surface(char *text, t_font *font);
void							render_text(t_libui *libui, SDL_Event e,
		t_widget *widgets);
void							set_window_position(t_libui *libui,
		char *title, t_vec2 pos);
