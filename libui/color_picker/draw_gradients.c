...
void draw_main_gradient(SDL_Surface *surface, double hue)
{
	...
	gradient = SDL_CreateRGBSurface(0, MAIN_GRADIENT_SIZE, MAIN_GRADIENT_SIZE, 32,
	...
	SDL_BlitSurface(gradient, &gradient->clip_rect, surface, &gradient->clip_rect);
...
void draw_hue_gradient(SDL_Surface *surface)
{
	...
	gradient = SDL_CreateRGBSurface(0, HUE_GRADIENT_W, HUE_GRADIENT_H, 32,
									0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000);
	y = 0;
	while (y < gradient->h)
	{
	...
	SDL_BlitSurface(gradient, NULL, surface, &(SDL_Rect){0, MAIN_GRADIENT_SIZE, 0, 0});
	SDL_FreeSurface(gradient);
}
